...
test.describe('Footer Copyright Notice', () => {
  test('should display copyright notice with current year', async ({ page }) => {
    ...
    await expect(footerText).toContainText('All Rights Are Reserved by Free For Charity a US 501c3 Non Profit');
  });
  ...
  test('should display link to freeforcharity.org in copyright notice', async ({ page }) => {
    ...
    const copyrightLink = page.locator('footer p:has-text("All Rights Are Reserved") a[href="https://freeforcharity.org"]');
